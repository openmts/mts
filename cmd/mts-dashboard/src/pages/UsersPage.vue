...
<template>
  <div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-lg font-semibold text-slate-800">用户</h1>
        <p class="text-xs text-slate-500">账号、角色与库权限</p>
      </div>
      <div class="flex gap-2">
        <button class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs text-slate-600 hover:bg-slate-50" @click="showChangeSelfPassword = true">
    ...
    <div v-if="!isAdmin" class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
    ...
    <div v-else class="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="u in users" :key="u.name" class="border-b border-slate-50 hover:bg-slate-50/40">
            <td class="px-4 py-3">
              <button class="text-left font-medium text-slate-800 hover:underline" @click="selectUser(u)">{{ u.name }}</button>
              <span v-if="u.display_name" class="ml-2 text-xs text-slate-400">{{ u.display_name }}</span>
            </td>
            <td class="px-4 py-3 text-slate-600">{{ u.role || 'user' }}</td>
            <td class="px-4 py-3">
              <span :class="u.disabled ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'" class="rounded px-2 py-0.5 text-xs">{{ u.disabled ? '禁用' : '正常' }}</span>
            </td>
            <td class="px-4 py-3">
              <div class="flex items-center gap-1">
                <button class="rounded p-1 text-slate-400 hover:text-slate-700" title="设置密码" @click="openSetPassword(u.name)"><Key class="h-4 w-4" /></button>
                <button class="rounded px-2 py-0.5 text-xs text-slate-500 hover:bg-slate-100" @click="toggleDisable(u)">{{ u.disabled ? '启用' : '禁用' }}</button>
                <button class="rounded p-1 text-slate-400 hover:text-red-600" @click="requestDelete(u.name)"><Trash2 class="h-4 w-4" /></button>
              </div>
            </td>
          </tr>
        </tbody>
