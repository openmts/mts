...
<template>
  ...
  <div v-else class="space-y-6">
    ...
    <div class="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900 md:grid-cols-5">
      <label class="text-xs text-slate-500">{{ t('user') }}
        <select v-model="selectedUser" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800">
          <option value="">全部用户</option>
          <option v-for="user in users" :key="user.name" :value="user.name">{{ user.display_name || user.name }}</option>
        </select>
      </label>
      <label class="text-xs text-slate-500">{{ t('action') }}
        <input v-model="actionFilter" placeholder="login / flush ..." class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800" />
      </label>
      <label class="text-xs text-slate-500">{{ t('since') }}
        <input v-model="sinceLocal" type="datetime-local" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800" />
      </label>
      <label class="text-xs text-slate-500">{{ t('until') }}
        <input v-model="untilLocal" type="datetime-local" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm dark:border-slate-600 dark:bg-slate-800" />
      </label>
      <div class="flex items-end gap-2">
        <button :disabled="loading" class="rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white disabled:opacity-50 dark:bg-slate-100 dark:text-slate-900" @click="loadAudit">
    ...
    <div class="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div class="flex items-center justify-between border-b border-slate-100 px-4 py-2 text-xs text-slate-500 dark:border-slate-800">
        <span class="inline-flex items-center gap-1"><ScrollText class="h-3.5 w-3.5" /> 审计事件</span>
        <span>{{ filteredCount }} 条</span>
      </div>
      <div v-if="!auditEvents.length" class="p-6 text-center text-sm text-slate-400">
      ...
      <table v-else class="w-full text-sm">
        <thead>
          <tr class="border-b border-slate-200 text-left dark:border-slate-800">
            ...
            <th class="px-4 py-3 text-xs font-medium text-slate-500">用户</th>
            <th class="px-4 py-3 text-xs font-medium text-slate-500">操作</th>
            <th class="px-4 py-3 text-xs font-medium text-slate-500">库</th>
        ...
        <tbody>
          <tr v-for="(evt, idx) in auditEvents" :key="idx" class="border-b border-slate-100 last:border-b-0 dark:border-slate-800">
            ...
            <td class="px-4 py-3 text-xs text-slate-500">{{ evt.database || '—' }}</td>
