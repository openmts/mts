    ...
    <div class="rounded-xl border border-slate-200 bg-white p-4 sm:p-6">
      ...
      <template v-else>
        ...
        <div class="space-y-2 sm:hidden">
          <div v-for="ec in errorCodes" :key="ec.code" class="rounded-lg border border-slate-200 p-3">
            <div class="mb-1 flex items-center justify-between">
              <span class="font-mono text-xs font-medium text-slate-800">{{ ec.code }}</span>
              <span class="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-600">HTTP {{ ec.http_status }}</span>
            </div>
            <p class="text-xs text-slate-500">{{ ec.description }}</p>
            <p class="mt-1 text-xs text-slate-400">gRPC: {{ ec.grpc_code }}</p>
          </div>
        </div>
      </template>
    </div>
