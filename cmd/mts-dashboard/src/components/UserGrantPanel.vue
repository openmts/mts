...
<template>
  <div class="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900 p-5">
    ...
    <div class="mb-3">
      <p class="mb-1 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">当前授权</p>
      <div v-if="!userGrants.length" class="text-xs text-slate-400 dark:text-slate-500">无库权限</div>
    ...
    <div class="grid gap-3 sm:grid-cols-2">
      <div>
        <p class="mb-1 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">数据库</p>
      ...
      <div>
        <p class="mb-1 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">权限</p>
        <div class="space-y-1 rounded border border-slate-100 p-2">
          <label v-for="perm in ['read', 'write', 'admin']" :key="perm" class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
