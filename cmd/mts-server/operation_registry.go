...
func operationCatalog() []operation {
	return []operation{
		...
		},
		{
			...
			ResponseHint: "Prometheus text exposition",
			Auth:         authNone,
			HTTPMethods:  []string{http.MethodGet},
		...
		// data
		{
			...
			ResponseHint: "writeResponse{ok,points,path,mode,database,retention_policy,admin_op_busy,op,started_at_unix,last}",
			Auth:         authDataToken,
		...
		},
		{
			...
			ResponseHint: "queryRowsResponse{rows,stats,row_count,path,database,measurement,admin_op_busy,op,started_at_unix,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "queryColumnsResponse{columns,stats,series_count,path,database,measurement,admin_op_busy,op,started_at_unix,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "queryExplainResponse{result,path,database,measurement,admin_op_busy,op,started_at_unix,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "NDJSON stream: row|column frames + end{path,format,record_count,database,measurement,stats,admin_op_busy,last} | error",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "deleteResponse{ok,path,database,measurement,admin_op_busy,op,started_at_unix,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		{
			Name:         "get_data_limits",
			Namespace:    "data",
			Description:  "read data-plane write/query limits (max_write_points/default_query_limit/max_query_limit)",
			ResponseHint: "dataLimitsResponse{max_write_points,default_query_limit,max_query_limit,path,admin_op_busy,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodGet},
			HTTPPaths:    []string{routeDataLimits},
			HTTPHandler:  (*serverRuntime).handleDataLimits,
			GRPCMethod:   grpcMethodGetDataLimits,
			GRPCRequest:  &emptyRequest{},
			GRPCFn:       grpcGetDataLimits,
		},
		{
			Name:         "get_data_contract",
			Namespace:    "data",
			Description:  "data-plane contract snapshot (limits + write/query/stream/delete meta capabilities)",
			ResponseHint: "dataContractResponse{version,path,max_write_points,default_query_limit,max_query_limit,features[{id,path,enabled}],admin_op_busy,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodGet},
			HTTPPaths:    []string{routeDataContract},
			HTTPHandler:  (*serverRuntime).handleDataContract,
			GRPCMethod:   grpcMethodGetDataContract,
			GRPCRequest:  &emptyRequest{},
			GRPCFn:       grpcGetDataContract,
		},
		{
			Name:         "list_data_databases",
			Namespace:    "data",
			Description:  "list databases readable by current user",
			ResponseHint: "databasesResponse{databases,path,admin_op_busy,last}",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "measurements|fields|series|retention list (+path,database,measurement,admin_op_busy/last)",
			Auth:         authDataToken,
			HTTPMethods:  []string{http.MethodGet},
		...
		// auth
		{
			...
			ResponseHint: "authTokenResponse{token,path,must_change_password,remaining_seconds,server_time_unix}",
			Auth:         authUserPass,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "okResponse{ok,admin_op_busy,last}",
			Auth:         authUserToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "changePasswordResponse{ok,path,must_change_password=false,admin_op_busy,last}",
			Auth:         authUserPass,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "sessionResponse{ok,path,user_name,role,expires_at,remaining_seconds,server_time_unix,must_change_password,admin_op_busy,last}",
			Auth:         authUserToken,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "passwordPolicyResponse{ok,path,min_length,forbidden_defaults,default_auth_ttl_seconds,max_auth_ttl_seconds,version}",
			Auth:         authNone,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "authzDatabaseCheckResponse{allowed,path}",
			Auth:         authUserToken,
			HTTPMethods:  []string{http.MethodPost},
		...
		// users (collection + resource)
		{
			...
			ResponseHint: "usersResponse|okResponse{path,admin_op_busy/last on GET/create}",
			Auth:         authAdmin,
		...
		},
		{
			...
			ResponseHint: "user|permissions|setPasswordResponse|okResponse{path,admin_op_busy/last; disable/enable writes user_disable|user_enable last}",
			Auth:         authAdmin,
		...
		},
		{
			Name:         "update_user",
			Namespace:    "users",
			Description:  "update user (disabling revokes tokens; response last=user_disable|user_enable)",
			ResponseHint: "okResponse{ok,admin_op_busy,last}",
			Auth:         authAdmin,
		...
		},
		{
			...
			ResponseHint: "batchMutationResponse{ok,path,ok_count,skip_count,fail_count,items,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "setPasswordResponse{ok,path,user_name,admin_op_busy,op,started_at_unix,last}",
		},
		{
		...
		// admin databases
		{
			...
			ResponseHint: "databasesResponse|okResponse (+admin_op_busy/last)",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet, http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "okResponse|retentionPoliciesResponse (+admin_op_busy/last)",
			Auth:         authAdmin,
		...
		// admin config / contract
		{
			...
			ResponseHint: "configResponse{config,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "configSchemaResponse{fields,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "configValidateResponse{ok,path,error?,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "reloadConfigResponse{ok,path,fields,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "apiSpecResponse{version,namespaces[].endpoints[].response?,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			Namespace:    "admin",
			Description:  "read error code contract (retryable/category/remediation/dashboard_path + admin_op_busy/last)",
			ResponseHint: "errorCodesResponse{codes[{code,http_status,grpc_code,description,retryable,category,remediation,dashboard_path}],path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "maintenanceErrorsResponse{errors,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "maintenanceStatsResponse{stats,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "opsStatusResponse{path,admin_op_busy,op,started_at_unix,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "storageMemoryResponse{snapshot,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "compactionStatsResponse{stats,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "doctorResponse{ok,path,checks,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "versionResponse{version,commit?,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "adminHealthResponse{path,health,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "storageValidateResponse{ok,path,data_dir,health,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "storageDataSnapshotResponse{path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "storageRestoreDrillResponse{ok,source,target,files,bytes,check_issues,check_fatals,check_root,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "storageDataSnapshotsResponse{path,snapshots,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			ResponseHint: "storageSnapshotResponse{path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodPost},
		...
		},
		{
			...
			ResponseHint: "storageSnapshotsResponse{path,snapshots,admin_op_busy,last} | okResponse{path} on DELETE",
			Auth:         authAdmin,
		...
		},
		{
			...
			ResponseHint: "auditListResponse{events,total,path,admin_op_busy,last}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		// downsample
		{
			...
			ResponseHint: "downsamplePoliciesResponse{path}|okResponse{path,admin_op_busy/last}",
			Auth:         authAdmin,
		...
		},
		{
			...
			Namespace:    "admin",
			Description:  "get/delete downsample policy, get status, or run policy actions",
			ResponseHint: "downsamplePolicyResponse{path}|downsamplePolicyStatusResponse{path}|okResponse{path} by policy action (+admin_op_busy/last)",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet, http.MethodPost, http.MethodDelete},
		...
		},
		{
			...
			ResponseHint: "downsampleStatusesResponse{path,statuses,summary,admin_op_busy,last; summary_only=1 omits rows}",
			Auth:         authAdmin,
			HTTPMethods:  []string{http.MethodGet},
		...
		},
		{
			...
			Namespace:    "admin",
			Description:  "dry-run downsample policy (response includes path/admin_op_busy/last)",
			ResponseHint: "downsampleDryRunResponse{result,path,admin_op_busy,last}",
			Auth:         authAdmin,
