...
稳定性、磁盘格式与 experimental 边界见 `docs/compatibility.md`。生产查询保护、删除回收与 Nightly 门禁见 `docs/ops/`。POC 存储路径使用列式 WAL（formatID=2）、SSTable metadata 内嵌组件 size，以及跨 shard 有界并行 compact。
