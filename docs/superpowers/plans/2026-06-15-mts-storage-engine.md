...
实现备注：已补充 `storagefs` 与 `wal` 错误路径/边界行为测试；`go test ./... -coverprofile=coverage.out -timeout 600s` 通过，总行覆盖率 `90.5%`，其中 `wal` 包覆盖率 `90.8%`；`goimports-reviser -project-name github.com/openmts/mts -recursive -format -rm-unused .`、`gofmt` 与 `golangci-lint run --timeout 12m` 均通过。当前仓库不存在 `tests/e2e` 目录，e2e 不适用；已删除 `coverage.out`，未发现 e2e 测试二进制产物。
