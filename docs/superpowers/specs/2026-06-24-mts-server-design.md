...
- When HTTP 客户端 POST `/api/v1/data/write` 时，系统应按请求中的 points 和 write options 写入数据。
- When HTTP 客户端 POST `/api/v1/data/query/rows` 时，系统应按结构化 Query 返回 rows，不引入 SQL/PromQL/InfluxQL parser。
- When HTTP 客户端 POST `/api/v1/admin/flush` 或 `/api/v1/admin/compact` 时，系统应执行对应维护动作并返回 JSON 结果。
